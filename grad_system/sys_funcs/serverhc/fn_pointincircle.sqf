...
// example: _random_point = [position trigger1, 200] call FWM_fn_pointincircle;
	...
	while {count _ret_val == 0 && _co < 50} do 
	{
        ...
        _point = [_center,(round (random _radius)),_angle] call IFUNC(relPos);
		_water = [_point] call SFUNC(watercheck);
		_hard_objs = [_point,5] call SFUNC(hardcheck);
